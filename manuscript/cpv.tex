...
distance and a single offset that segments the space into intervals.
If a given probe is 40 bases away from two (or more) other probes, then it will
appear more than once in the interval that contains probe-pairs at
40 bases. This is useful in cases where the probes generating the p-values
...
correction or on a null-model may then be calculated. The peak-finding
...
A p-value for each region can be assigned using the Stouffer-Liptak
correction.
...
the parameter-choice, we show that the Irizarry DMR's that overlap
...
Finally, we show that our method yields no false-positive DMR's on shuffled
CHARM data and that it recovers more significant probes than doing a simple
FDR correction on the original p-values.
...
samples to obtina p-values from a one-sided moderated t-test.
...
In addition, \textit{comb-p} also have a higher target enrichment
